...
//
#include <iostream>
#include "dji_sdk_demo/linear_quadratic_regulator.h"
...
/* Set Fixed Matrix */
void LQR::setFixed( Eigen::MatrixXf _A, Eigen::MatrixXf _B, Eigen::MatrixXf _Q, Eigen::MatrixXf _R ){
    A = _A;
    B = _B;
    Q = _Q;
    R = _R;
...
void LQR::solver( void ){
    ...
    Eigen::MatrixXf tmp = B.transpose() * S * B + R;
...
void check_stabilizable(const Eigen::Ref<const Eigen::MatrixXf>& A,
...
void check_detectable(const Eigen::Ref<const Eigen::MatrixXf>& A,
  ...
  int n = A.rows();
  Eigen::LDLT<Eigen::MatrixXf> ldlt(Q);
  Eigen::MatrixXf L = ldlt.matrixL();
  Eigen::MatrixXf D = ldlt.vectorD();
  Eigen::MatrixXf D_sqrt = Eigen::MatrixXf::Zero(n, n);
  for (int i = 0; i < n; i++) {
    D_sqrt(i, i) = sqrt(D(i));
  }
  Eigen::MatrixXf C = L * D_sqrt;
...
void Givens_rotation(float a, float b, Eigen::Ref<Eigen::Matrix2d> R,
                     float eps = 1e-10) {
  float c, s;
  if (fabs(b) < eps) {
    c = (a < -eps ? -1 : 1);
    ...
    float t = b / a;
    float u = sgn(a) * fabs(sqrt(1 + t * t));
    c = 1 / u;
    s = -c * t;
  } else {
    float t = a / b;
    float u = sgn(b) * fabs(sqrt(1 + t * t));
    s = -1 / u;
    c = -s * t;
  }
...
void swap_block_11(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                   Eigen::Ref<Eigen::MatrixXf> Z, int p) {
  // Dooren, Case I, p124-125.
  int n2 = S.rows();
  Eigen::Matrix2d A = S.block<2, 2>(p, p);
  Eigen::Matrix2d B = T.block<2, 2>(p, p);
  Eigen::MatrixXf Z1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::Matrix2d H = A(1, 1) * B - B(1, 1) * A;
  Givens_rotation(H(0, 1), H(0, 0), Z1.block<2, 2>(p, p));
  S = (S * Z1).eval();
  T = (T * Z1).eval();
  Z = (Z * Z1).eval();
  Eigen::MatrixXf Q = Eigen::MatrixXf::Identity(n2, n2);
...
// The arguments S, T, and Z will be changed.
void swap_block_21(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                   Eigen::Ref<Eigen::MatrixXf> Z, int p) {
  ...
  Eigen::Matrix3d R = Eigen::MatrixXf::Identity(3, 3);
  Givens_rotation(H(0, 0), H(1, 0), R.block<2, 2>(0, 0));
  H = (R * H).eval();
  Eigen::MatrixXf Z1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Z2 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q2 = Eigen::MatrixXf::Identity(n2, n2);
  // Compute Z1, Z2, Q1, Q2.
...
// The arguments S, T, and Z will be changed.
void swap_block_12(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                   Eigen::Ref<Eigen::MatrixXf> Z, int p) {
  int n2 = S.rows();
  // Swap the role of S and T.
  Eigen::MatrixXf Z1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Z2 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q0 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q2 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Q3 = Eigen::MatrixXf::Identity(n2, n2);
...
// The arguments S, T, and Z will be changed.
void swap_block_22(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                   Eigen::Ref<Eigen::MatrixXf> Z, int p) {
  ...
  // W. Demmelt;
  int n2 = S.rows();
  Eigen::MatrixXf A = S.block<4, 4>(p, p);
  Eigen::MatrixXf B = T.block<4, 4>(p, p);
  // Solve
  // A11 * X - Y A22 = A12
  // B11 * X - Y B22 = B12
  // Reduce to solve Cx=D, where x=[x1;...;x4;y1;...;y4].
  Eigen::Matrix<float, 8, 8> C = Eigen::Matrix<float, 8, 8>::Zero();
  Eigen::Matrix<float, 8, 1> D;
  // clang-format off
  ...
  Eigen::MatrixXf x = C.colPivHouseholderQr().solve(D);
  // Q * [ -Y ] = [ R_Y ] ,  Z' * [ -X ] = [ R_X ] .
  //     [ I  ]   [  0  ]         [ I  ] = [  0  ]
  Eigen::Matrix<float, 4, 2> X, Y;
  X << -x(0, 0), -x(1, 0), -x(2, 0), -x(3, 0), Eigen::Matrix2d::Identity();
  Y << -x(4, 0), -x(5, 0), -x(6, 0), -x(7, 0), Eigen::Matrix2d::Identity();
  Eigen::MatrixXf Q1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::MatrixXf Z1 = Eigen::MatrixXf::Identity(n2, n2);
  Eigen::ColPivHouseholderQR<Eigen::Matrix<float, 4, 2> > qr1(X);
  Z1.block<4, 4>(p, p) = qr1.householderQ();
  Eigen::ColPivHouseholderQR<Eigen::Matrix<float, 4, 2> > qr2(Y);
...
void swap_block(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                Eigen::Ref<Eigen::MatrixXf> Z, int p, int q, int q_block_size,
                float eps = 1e-10) {
...
void reorder_eigen(Eigen::Ref<Eigen::MatrixXf> S, Eigen::Ref<Eigen::MatrixXf> T,
                   Eigen::Ref<Eigen::MatrixXf> Z, float eps = 1e-10) {
  // abs(a) < eps => a = 0
  int n2 = S.rows();
  int n = n2 / 2, p = 0, q = 0;
  while (p < n && q < n2) {
    ...
    while (q < n2) {
      if (q == n2 - 1 || fabs(S(q + 1, q)) < eps) {  // block size = 1
        ...
        float det_S = S(q, q) * S(q + 1, q + 1) - S(q + 1, q) * S(q, q + 1);
        float det_T = T(q, q) * T(q + 1, q + 1) - T(q + 1, q) * T(q, q + 1);
...
Eigen::MatrixXf DiscreteAlgebraicRiccatiEquation(
  ...
  Eigen::MatrixXf M(2 * n, 2 * n), L(2 * n, 2 * n);
  M << A, Eigen::MatrixXf::Zero(n, n), -Q, Eigen::MatrixXf::Identity(n, n);
  L << Eigen::MatrixXf::Identity(n, n), B * R.inverse() * B.transpose(), Eigen::MatrixXf::Zero(n, n), A.transpose();
